...
#include "Weapon/LyraGameplayAbility_RangedWeapon.h"
#include "LyraRangedWeaponInstance.h"
#include "AbilitySystemComponent.h"
#include "Physics/LyraCollisionChannels.h"
#include <AbilitySystem/LyraGameplayAbilityTargetData_SingleTarget.h>

ULyraGameplayAbility_RangedWeapon::ULyraGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
}
...
int32 FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
	for (int32 Idx = 0; Idx < HitResults.Num(); ++Idx)
	{
		...
		else
		{
			...
			// 한단계 AttachParent에 Actor가 Pawn이라면? 캐릭터를 헬멧을 장착해서 맞은 경우를 상정
...
FHitResult ULyraGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, TArray<FHitResult>& OutHitResults) const
{
	...
	if (HitResults.Num() > 0)
	{
		...
		// Hit의 가장 마지막 값을 Impact로 저장, 가장 마지막에 히트된 오브젝트를 기준으로 히트효과를 연출해야줘야 하기 때문
